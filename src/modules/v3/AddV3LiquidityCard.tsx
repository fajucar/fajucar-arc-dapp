...
export function AddV3LiquidityCard({ onMintSuccess }: AddV3LiquidityCardProps = {}) {
  ...
  const token1 = tokenA && tokenB && token1Addr ? (token1Addr.toLowerCase() === tokenA.address.toLowerCase() ? tokenB : tokenA) : null
    ...
    publicClient
      ...
      .then((addr) => {
        ...
        if (a && a !== '0x0000000000000000000000000000000000000000') {
          ...
          setPoolAddress(null)
          setPoolError('Pool not available. Only USDC/EURC has a deployed V3 pool on Arc Testnet.')
        }
      })
      .catch(() => {
        setPoolAddress(null)
        setPoolError('Error checking pool.')
      })
  ...
  const handleMint = async () => {
    if (!address || !publicClient || !positionManager || !token0Addr || !token1Addr || !addrs) {
      toast.error('Connect wallet and ensure V3 config.')
      return
    }
    ...
    try {
      toast.loading('Confirme na sua carteira (MetaMask)...', { id: toastId })
                ...
                <div className="space-y-6">
                  ...
                  <div className="rounded-2xl border border-slate-700/50 bg-slate-800/30 p-5">
                    ...
                    <p className="text-xs text-slate-400 mb-4">
                      Current price: {currentPrice || '—'} {symbol1} per {symbol0}
                    </p>
                    {!inRange && (
                      <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs mb-4">
                        <span>⚠</span>
                        <span>You are about to deposit outside of the current price range. {symbol1} may not be needed for this deposit.</span>
                      </div>
                    )}
                    ...
                    <div className="mt-3 flex gap-4 text-xs text-slate-500">
                      <span>Min: {minPrice.toFixed(4)} · Tick: {Math.min(tickLower, tickUpper)}</span>
                      <span>Max: {maxPrice.toFixed(4)} · Tick: {Math.max(tickLower, tickUpper)}</span>
                    </div>
                  </div>
                    ...
                    <div className="space-y-4">
                      <div className="rounded-xl bg-slate-900/60 border border-slate-700/50 p-4">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="text-xs text-slate-500">Balance {formatNumber(formatUnits(balance0, decimals0), 4)}</span>
                        </div>
                        <input
                          type="text"
                          inputMode="decimal"
                      ...
                      <div className="rounded-xl bg-slate-900/60 border border-slate-700/50 p-4">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="text-xs text-slate-500">Balance {formatNumber(formatUnits(balance1, decimals1), 4)}</span>
                        </div>
                        <input
                          type="text"
                          inputMode="decimal"
                      ...
                      <button
                        ...
                        className="w-full py-3.5 rounded-xl font-semibold bg-cyan-500 hover:bg-cyan-600 text-white disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-colors"
                      >
                        {isLoading ? (
                          <>
