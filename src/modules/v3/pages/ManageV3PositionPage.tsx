...
export function ManageV3PositionPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (!cancelled) {
        setBalance0(formatUnits(b0, decimals0))
        setBalance1(formatUnits(b1, decimals1))
      }
    }).catch(() => {
      if (!cancelled) setBalance0(''); setBalance1('')
    })
  ...
  const increaseLiquidity = async () => {
    ...
    try {
      if (a0 > 0n) {
        toast.loading(`Approving ${sym0}...`, { id: 'ap0' })
        await ensureAllowance(publicClient, writeOpts, pos.token0, address, addrs.v3PositionManager, a0)
        toast.dismiss('ap0')
      }
      if (a1 > 0n) {
        toast.loading(`Approving ${sym1}...`, { id: 'ap1' })
        await ensureAllowance(publicClient, writeOpts, pos.token1, address, addrs.v3PositionManager, a1)
        toast.dismiss('ap1')
      }
  ...
  const decreaseLiquidity = async () => {
    ...
    try {
      ...
      await publicClient.waitForTransactionReceipt({ hash: h })
      toast.dismiss('dec')
      toast.loading('Collecting tokens...', { id: 'col' })
      ...
      toast.dismiss('col')
      toast.success('Liquidity decreased')
      setDecreasePct(null)
    } catch (e) {
      setTxState('error')
      toast.dismiss()
  ...
  if (isWrongChain || configError) {
    return (
      <div className="py-8 px-4 max-w-3xl mx-auto">
        <Link to="/pools?tab=v3" className="inline-flex items-center gap-2 text-cyan-400 hover:text-cyan-300 mb-4">
    ...
    return (
      <div className="py-8 px-4 max-w-3xl mx-auto">
        ...
        <div className="p-8 text-center rounded-2xl border border-slate-700/50 bg-slate-800/20">
          ...
          <p className="text-slate-400">Loading position #{tokenId}...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="py-8 px-4 max-w-5xl mx-auto">
      <Link to="/pools?tab=v3" className="inline-flex items-center gap-2 text-cyan-400 hover:text-cyan-300 mb-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          {/* Manage position */}
          <div id="collect" className="rounded-2xl border border-slate-700/50 bg-slate-800/30 p-6 scroll-mt-4">
            <div className="flex items-start justify-between mb-4">
              <h2 className="text-lg font-semibold text-white">Manage position</h2>
              <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-lg text-xs font-medium ${inRange ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'}`}>
                <span className={`w-2 h-2 rounded-full ${inRange ? 'bg-emerald-400' : 'bg-amber-400'}`} />
                {inRange ? 'In range' : 'Out of range'}
              </span>
            </div>
            <p className="text-slate-400 text-sm mb-1">Position #{tokenId}</p>
            <p className="text-slate-300 text-sm font-medium mb-4">{priceLow.toFixed(4)} ⇌ {priceHigh.toFixed(4)} {sym1}</p>
            <div className="grid grid-cols-3 gap-3 text-sm mb-4">
              <div>
                <span className="text-slate-500 block">APR</span>
                <span className="text-slate-200">0.00%</span>
              </div>
              <div>
                <span className="text-slate-500 block">Staked</span>
                <span className="text-slate-200">$—</span>
              </div>
              <div>
                <span className="text-slate-500 block">Deposited</span>
                <span className="text-slate-200">
                  {formatNumber(formatUnits(posAmount0, decimals0), 2)} {sym0} + {formatNumber(formatUnits(posAmount1, decimals1), 2)} {sym1}
                </span>
              </div>
            </div>
            {(pos.tokensOwed0 > 0n || pos.tokensOwed1 > 0n) && (
              <p className="text-xs text-amber-400 mb-4">
                Rewards: {formatNumber(formatUnits(pos.tokensOwed0, decimals0), 4)} {sym0} / {formatNumber(formatUnits(pos.tokensOwed1, decimals1), 4)} {sym1}
              </p>
            )}
            <div className="flex flex-wrap gap-3">
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold bg-cyan-500 hover:bg-cyan-600 text-white disabled:opacity-50 disabled:cursor-not-allowed"
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium border border-cyan-500/50 bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20"
              >
              ...
              {pos.liquidity > 0n && (
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium border border-slate-600 bg-slate-800/60 text-slate-200 hover:bg-slate-700/60"
                >
          ...
          <div className="rounded-2xl border border-slate-700/50 bg-slate-800/30 p-6">
            ...
            <p className="text-xs text-slate-400 mb-4">Current price: {curPrice.toFixed(4)} {sym1} per {sym0}</p>
            {!inRange && (
              <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs mb-4">
                <span>⚠</span>
                <span>Your range does not include the active tick. This is a single-sided position.</span>
              </div>
            )}
            <div className="flex gap-2 mb-4">
              <button
                onClick={() => setRangeMode('full')}
                className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-colors ${rangeMode === 'full' ? 'bg-cyan-500/30 text-cyan-300 border border-cyan-500/50' : 'bg-slate-800/60 text-slate-400 border border-slate-600'}`}
              >
                Full range
              </button>
              <button
                onClick={() => setRangeMode('manual')}
                className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-colors ${rangeMode === 'manual' ? 'bg-cyan-500/30 text-cyan-300 border border-cyan-500/50' : 'bg-slate-800/60 text-slate-400 border border-slate-600'}`}
              >
                Manual
              </button>
            </div>
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-900/60 border border-slate-600 rounded-xl px-4 py-2.5 text-white text-sm"
                  />
                </div>
                <div className="flex-1">
            ...
            {rangeMode === 'manual' && (!rangeValid || !manualAligned) && (
              <p className="text-xs text-amber-400 mt-2">{!rangeValid ? 'Min &lt; Max' : `Ticks × ${sp}`}</p>
            )}
          </div>
          ...
          <div className="rounded-2xl border border-slate-700/50 bg-slate-800/30 p-6">
            ...
            <div className="space-y-4">
              <div className="rounded-xl bg-slate-900/60 border border-slate-700/50 p-4">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setAmount0(v)
                    if (v) setAmount1(computeAmount1From0(v))
                  }}
                  placeholder="0"
              ...
              <div className="rounded-xl bg-slate-900/60 border border-slate-700/50 p-4">
                ...
                <input
                  ...
                  onChange={(e) => {
                    const v = e.target.value.replace(/,/g, '.')
                    setAmount1(v)
                    if (v) setAmount0(computeAmount0From1(v))
                  }}
                  placeholder="0"
                  className="w-full bg-transparent text-xl font-semibold text-white focus:outline-none"
                />
              ...
              <button
                ...
                className="w-full py-3.5 rounded-xl bg-cyan-500 hover:bg-cyan-600 text-white font-semibold disabled:opacity-50 flex items-center justify-center gap-2"
              >
          ...
          {/* Decrease */}
          {pos.liquidity > 0n && (
            <div id="decrease" className="rounded-2xl border border-slate-700/50 bg-slate-800/30 p-6 scroll-mt-4">
              <h3 className="text-sm font-semibold text-white mb-4">Decrease liquidity</h3>
              <p className="text-xs text-slate-400 mb-3">Select how much to remove:</p>
              <div className="flex flex-wrap gap-2 mb-3">
                {([25, 50, 75, 100] as const).map((p) => (
                  <button
                    key={p}
                    onClick={() => setDecreasePct(decreasePct === p ? null : p)}
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${decreasePct === p ? 'bg-red-500/30 text-red-400 border-2 border-red-500/60' : 'bg-slate-800/60 text-slate-400 border border-slate-600 hover:bg-slate-700/60'}`}
                  >
                    {p}%
                  </button>
                ))}
              ...
              <button
                ...
                className="w-full py-3 rounded-xl bg-red-500/20 hover:bg-red-500/30 text-red-400 font-semibold border border-red-500/40 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {busy ? <Loader2 className="h-5 w-5 animate-spin" /> : <Minus className="h-5 w-5" />}
